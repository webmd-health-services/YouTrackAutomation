...
BeforeAll {
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'YouTrackAutomation'|
...
Describe 'YouTrackAutomation' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_YouTrackAutomation' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'YouTrackAutomation' -CommandType Function,Cmdlet,Filter))
        {
